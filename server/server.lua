...
function CountyCooldown()
  ...
  if countycooldown <= 0 then
    ...
    TriggerClientEvent("chat:addMessage", -1, {
      color = {255, 0, 0},
      multiline = true,
      args = {"~b~SimplePrio", "~r~NOTICE: ~y~County Cooldown timer expired! Priorities are available again!"}
    })
    return
  end
end
...
function CityCooldown()
  if citycooldown ~= 0 then
    ...
    while citycooldown > 0 do 
      cityprio = Config.CountyPriority.Text.CooldownText .. ' (' .. citycooldown .. ' Minutes remaining!)'
  ...
  if citycooldown <= 0 then
    ...
    cityprio = Config.CountyPriority.Text.AvailableText
    TriggerClientEvent('Client:Sync', -1, 'cityprio', cityprio)
    TriggerClientEvent("chat:addMessage", -1, {
      color = {255, 0, 0},
      multiline = true,
      args = {"~b~SimplePrio", "~r~NOTICE: ~y~City Cooldown timer expired! Priorities are available again!"}
    })
    return
  end
end
...
RegisterCommand('county-pcd', function(source, args, rawCommand)
  TriggerClientEvent("chat:addMessage", -1, {
    color = {255, 0, 0},
    multiline = true,
    args = {"~b~SimplePrio", "~r~NOTICE: ~y~A priority in the " .. Config.CountyPriority.Basic.ZoneName .. " was ended by " .. GetPlayerName(source) .. '! Cooldown is now in effect!'}
  })
...
RegisterCommand('city-pcd', function(source, args, rawCommand)
  TriggerClientEvent("chat:addMessage", -1, {
    color = {255, 0, 0},
    multiline = true,
    args = {"~b~SimplePrio", "~r~NOTICE: ~y~A priority in the " .. Config.CityPriority.Basic.ZoneName .. " was ended by " .. GetPlayerName(source) .. '! Cooldown is now in effect!'}
  })
...
-- Peacetime System --
  ...
  if Config.Peacetime.Basic.Framework == "nat" then 
    ...
    function checkPerms(src)
      local player = exports[Config.Peacetime.Basic.NATFramework_Name]:getdept(src)
      if player then
        if(player[src].level == Config.Peacetime.NAT2K15_Settings.NAT2K15_Departments.State_Level or player[src].level == Config.Peacetime.NAT2K15_Settings.NAT2K15_Departments.Police_Level or player[src].level == Config.Peacetime.NAT2K15_Settings.NAT2K15_Departments.Sheriff_Level) then
          return true
        else 
          return false
        end
  ...
  local player = source
    if IsPlayerAceAllowed(source, 'pt.admin') then
      if peacetimeon then
        ...
        peacetimeon = true
        TriggerEvent("SimplePrio:peacetime:Sync", peacetimeon)
        if Config.Peacetime.Basic.Framework == 'nat' or Config.Peacetime.Basic.Framework == 'nd' then
